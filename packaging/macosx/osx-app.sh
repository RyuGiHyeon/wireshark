...
qt_frameworks_dir=$( "/opt/homebrew/bin/qmake" -query QT_INSTALL_LIBS )
...
sparkle_version="0"
sparkle_frameworks_dir="SPARKLE_LIBRARY-NOTFOUND"
...
pkgplugin="$bundle/Contents/PlugIns/$app_lower/4-7"
...
bundle_binary_rpaths=("$LIBPREFIX/lib")
rpaths=()
...
while $endl; do
	...
	while read -r lib ; do
		...
		| grep -v @rpath \
		| cut -d\( -f1 \
		| sed "1,\$s;^	libbrotli;	$LIBPREFIX/lib/libbrotli;" \
		| sed "1,\$s;^	@loader_path/libbrotli;	$LIBPREFIX/lib/libbrotli;" \
...
"/opt/homebrew/bin/macdeployqt" "$bundle" -no-strip -verbose=2 || exit 1
...
codesign_file () {
	...
	codesign \
		...
		--force \
		--options runtime \
		--entitlements "/Users/jeff/dev/wireshark/packaging/macosx/entitlements.plist" \
		--timestamp \
		--verbose \
		"$1" || exit 1
}
